...
/**
 * Verificación 2.1: Conexión a MongoDB
 */
...
/**
 * Verificación 6: Scripts en package.json
 */
async function verifyScripts() {
  const backendPackage = path.join(__dirname, 'backend', 'server', 'package.json');
  ...
  try {
    ...
    if (!backendPkg.scripts || !backendPkg.scripts.start) {
      throw new Error('Script "start" no encontrado en package.json del backend');
    }

    const frontendPkg = JSON.parse(fs.readFileSync(frontendPackage, 'utf8'));
    if (!frontendPkg.scripts || !frontendPkg.scripts.dev) {
      throw new Error('Script "dev" no encontrado en package.json del frontend');
    }

    console.log('[OK] Scripts de package.json verificados');
    return true;
  } catch (error) {
    throw new Error(`Error verificando scripts: ${error.message}`);
  }
}
...
export async function verifyConfiguration(includeHealthCheck = false) {
  ...
  const checks = [
    ...
    { name: 'Conexión MongoDB', func: verifyDatabaseConnection },
    { name: 'Scripts package.json', func: verifyScripts },
    // { name: 'Usuarios por defecto', func: verifyAndCreateDefaultUsers }, // Temporalmente comentado por timeout
